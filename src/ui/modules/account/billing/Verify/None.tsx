import React from 'react';
import { FaIdCard } from 'react-icons/fa';
import { useGetMessage } from 'ui/intl';
import { ids } from 'ui/messages';
import FormError from 'ui/elements/FormError';
import UploadId from './UploadId';
import type { Status } from '@respite/core';

export default function None({
  error,
  status,
  onChange,
}: {
  error: any;
  status: Status;
  onChange(file: File): any;
}) {
  const g = useGetMessage();

  return (
    <div className="space-y-8">
      <h3 className="text-lg font-bold flex space-x-4 items-center">
        <FaIdCard size="2em" />
        <span>{g(ids.account.billing.verify.title)}</span>
      </h3>
      <p>{g(ids.account.billing.verify.none.description)}</p>
      <p>{g(ids.account.billing.verify.none.action)}</p>
      <div>
        <FormError error={error} />
      </div>
      <div className="w-72 mx-auto">
        <UploadId onChange={onChange} status={status} />
      </div>
      <p className="italic">{g(ids.account.billing.verify.none.hint)}</p>
    </div>
  );
}
